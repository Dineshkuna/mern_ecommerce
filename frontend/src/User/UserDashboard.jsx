...
function UserDashboard({user}) {
 ...
 function toggleMenu(){
   ...
   function logoutUser(){
    ...
    .catch((error) => {
      toast.error(error.message || 'Logout failed. Please try again later.', {position:'top-center', autoClose:3000});
    });
   }
    function dashboard(){
  ...
  return (
    ...
    <div className='dashboard-container'>
        <div className="profile-header " onClick={toggleMenu}>
          ...
          alt="Profile Picture" className='profile-avatar'
        />
        <span className="profile-name">{user.name}</span>
        </div>
        {menuVisible && (<div className="menu-options">
          { options.map((item) => (<button  key ={item.name}className={`menu-option-btn ${item.isCart?(cartItems.length>0?'cart-not-empty':''):''}`} onClick={item.funcName}>{item.name}</button>))}
        </div>)}
